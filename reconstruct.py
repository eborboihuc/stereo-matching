...
class Reconstructor():
    ...
    def __init__(self, img_l, img_r, disp, focal_length, baseline, show_disp_map=False, show_depth_map=False, trans_speed=50, angle_speed=np.pi/180):
        self.img_l = img_l
        self.img_r = img_r
        self.disp = disp
    ...
    def _calc_point_cloud(self):
        points = cv2.reprojectImageTo3D(self.disp, self.q, handleMissingValues=True).reshape(-1, 3)
    ...
    def _project_points(self, r, t):
        projected, _ = cv2.projectPoints(self.points, r, t, self.k, self.dist_coeff)
        xy = projected.reshape(-1, 2).astype(np.int)
        mask = (
            (0 <= xy[:, 0]) & (xy[:, 0] < self.width) &
            (0 <= xy[:, 1]) & (xy[:, 1] < self.height)
        )
...
def read_depth(path):
    ...
    assert depth.ndim == 2, "Depth {} has more ndim ({}) than 2".format(path, depth.ndim)
    return depth


def triangulation(disp):
    # Get depth from disp
    depth = 0.54 * 721.5377 / disp
    depth[disp<=1e-4] = 0
